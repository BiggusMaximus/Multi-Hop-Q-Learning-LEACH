...
def get_single_dict(dic):
    # Return the next state node based on the lowest connected Q-Value
    single_link = {}
    min_value = min(dic.values())
    for key in dic.keys():
        if dic[key] == min_value:
            single_link[key] = dic[key]
    print(f"dic : {dic} | min_value : {min_value} | single_link.keys() ; {single_link.keys()}")
...
def get_best_nodes(Q,start,end):
    ...
    while list(set(next_level) & set(end)) == []:
        print(f"list(set(next_level) & set(end)) : {list(set(next_level) & set(end))} | next_level : {next_level}  | set_next_level : {set(next_level)} | end : {end}")
        temp_level = []
        for i in next_level:
