...
class Node:
    ...
    def energy_contention_TDMA_CH(self, Nc):
        return (kc * Nc * E_elec ) + kc * (E_elec + P_tx * attenuation(self.z, d_max, show_info = False))
    
    def energy_contention_TDMA_Node(self, Nc, d_CH_Node):
        a = ratio_delay(self.z, d_CH_Node)
        rho = throughput(a)
        energy = (kc / rho) * (E_elec + P_tx * attenuation(self.z, d_CH_Node, show_info = False)) + ((Nc-1)/ rho) * kc * self.energyIdle() + kt * E_elec
        return energy
    
    def energyFrame_CH(self, Nc, d_CH_BS):
        return (m * Nc * kd * E_elec) + (m * (N-Nc) * self.energyIdle() * kd) + kd * (E_elec + P_tx * attenuation(self.z, d_CH_BS, show_info = False))
    
    def energyFrame_Node(self, d_CH_Node):
        return m * kd * (E_elec + P_tx * attenuation(self.z, d_CH_Node, show_info = False))
...
class networkEnvironment:
    ...
    def startSimulation(self, rounds):
        ...
        params = {
            ...
            'node_failure' : self.node_failure,
            'PDR' : self.packet_delivered,
            'setup' : self.setupPhaseEnergy,
            'steady' : self.steadyPhaseEnergy,
        }
        return params
                    

# %% [markdown]
# # Multi layer Trilateration
...
node_trilateration    = createNetworks()
LEACH_trilateration = networkEnvironment(node_trilateration, "LEACH", False, 500, QL_params)
LEACH_trilateration_params = LEACH_trilateration.startSimulation(500)
...
node_random    = createNetworksRandom()
LEACH_random = networkEnvironment(node_random, "LEACH", False, 500, QL_params)
LEACH_random_params = LEACH_random.startSimulation(500)

# %%
params = len(LEACH_trilateration_params.keys())
...
count = 0
for param in LEACH_trilateration_params.keys():
    rounds = [i for i in range(len(LEACH_trilateration_params['alive']))]
    ax[count].plot(rounds, LEACH_random_params[param])
    ax[count].plot(rounds, LEACH_trilateration_params[param])
    ax[count].scatter(rounds, LEACH_random_params[param]             , edgecolors='k' , label="Random")
    ax[count].scatter(rounds, LEACH_trilateration_params[param]      , edgecolors='k' , label="Proposed Method")
